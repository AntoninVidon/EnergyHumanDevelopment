...
ggplot(graph_df, aes(x=year, y=renew, group=income_group, color=income_group)) +
  ...
  scale_colour_discrete(labels = c('High income (>12,535 USD)', 
                                   'Upper middle income (4,046 - 12,535 USD)', 
                                   'Lower middle income (1,036- 3,995 USD)'))
```
</br>
...
# graph_df <- master_df %>% filter(!is.na(renew))
# graph_df$income_group[graph_df$country=="Venezuela"] <- "Lower middle income"
# graph_df <- graph_df[graph_df$income_group == "High income",]
# graph_df <- graph_df %>% mutate(renew = renew / primary*100)
# 
# ggplot(graph_df, aes(x=year, y=renew, group=country, color=income_group)) +
#   geom_line(key_glyph="timeseries", size=0.5) +
#   theme_classic() + 
#   geom_text(
#     label=graph_df$country, 
#     nudge_x = 0, nudge_y = 0.02,
...
#   ) +
#   theme(panel.grid.major.y = element_line()) +
#   labs(x="Year", y="Share of renewable energy (in %)", color="Income Groups", caption="No data available for lowest income", title="Growth of Renewable Energy by Income Group") +
#   scale_colour_discrete(labels = c('High income (>12,535 USD)', 'Upper middle income (4,046 - 12,535 USD)', 'Lower middle income (1,036- 3,995 USD)'))
```