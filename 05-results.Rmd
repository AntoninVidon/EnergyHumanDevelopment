...
df_energy <- df_energy %>% 
  select(-contains(c("_per_capita", "_pct", "_change", "_production"))) %>%
...
df_schooling$country[df_schooling$country == "Côte d'Ivoire"] <- "Cote d'Ivoire"
...
```{r}
df_energy_and_schooling <- merge(df_energy, df_schooling, by = c("country", "year"), all.x = TRUE)
```

Is an energy mix oriented towards renewable sources associated with good human development indicators? 

```{r}
df_graph <- df_energy_and_schooling[df_energy_and_schooling$year == 2019,]
ggplot(df_graph, aes(x=low_carbon_share_elec, y=schooling_years)) +
  geom_point(aes(color = year)) +
  geom_smooth(method="lm") +
  theme_light() + 
  geom_text(
    label=df_graph$country, 
    nudge_x = 0.25, nudge_y = 0.3,
    size = 3,
    check_overlap = T
  )
```
...
pairs(df_energy_and_schooling[df_energy_and_schooling$year == 2019, selected_ids])
```
...
knitr::kable(head(master_df, 10), "html")
plot_missing_parameters(master_df, use_counts = FALSE, max_100 = FALSE, 
                        ticks_angle = 45, ticks_size = 9, complete_cases_size = 3, complete_cases_hjust = 0.3, y_ticks_size = 5)
```
