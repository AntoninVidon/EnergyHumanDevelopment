...
ggplot(graph_df, aes(x=renew/primary*100, y=HDI)) +
...
```{r}
graph_df <- master_df[master_df$year==2019,]
ggplot(graph_df, aes(x=renew/population*1000000000, y=HDI)) +
  geom_point(aes(color = region)) +
  geom_smooth(method="lm") +
  theme_light() + 
  geom_text(
  ...
  scale_x_log10() +
  labs(x="Renewable energy consumption per capita (in MWh)", color="Region")
```
...
perc_df_2019 <- perct_df %>% filter(year==2019)
