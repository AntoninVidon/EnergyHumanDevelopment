...
### An overview of HDI in the world
...
# plot HDI on a map in 2019
map_df <- master_df %>%
  filter(year == 2019) %>%
  mutate(ratio_primary_population = primary/population)

sPDF <- joinCountryData2Map(map_df,
                            joinCode = "ISO3",
                            nameJoinColumn = "ISO_code",
                            mapResolution = "coarse")

mapCountryData(sPDF, mapTitle = "Human Development Index in 2019", nameColumnToPlot="HDI", addLegend = TRUE)
```
...
There seems to be a **threshold of energy consumption per capita which separates two types of HDI behaviors**. For countries with low level of energy consumption, the causal relationship between energy consumption and HDI is very strong, as suggested by the high value of $R^2$. For countries with high level of energy consumption, there does not seem to be a correlation between both variables. This might be due to the fact that, when becoming wealthier, a country directs most of its financing power to increase proxies of human development 
