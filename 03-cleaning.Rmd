...
df_GDP_wide <- read.csv("data/GDP_per_capita.csv", skip = 4, check.names = FALSE)
df_GDP_wide <- df_GDP_wide %>%
  select("Country Code", relevant_years) %>%
...
df_schooling_wide <- df_schooling_wide %>%
  ...
  select("ISO_code", "HDI_rank", relevant_years)
...
common_ISO_codes <- Reduce(intersect, list(df_energy_long$ISO_code,df_life_wide$ISO_code, df_schooling_wide$ISO_code, df_GDP_wide$ISO_code))
...
df_schooling_long <- df_schooling_wide %>% 
  pivot_longer(cols = -c("ISO_code", "HDI_rank"), names_to = "year", values_to = "years_of_schooling") %>%
...
master_df <- merge(df_energy_long, 
                   merge(df_GDP_long, 
                         merge(df_life_long, df_schooling_long, 
...
master_df <- master_df %>%
  ...
  select(ISO_code, country, year, HDI_rank, GDP, life_expectancy, years_of_schooling, everything())
```
